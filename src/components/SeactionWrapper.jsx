...
const SeactionWrapper = ({ title, description, showBtn, mockupImg, banner, reverse }) => {
  return (
            ...
            <div className={`${styles.descDiv} ${reverse ? "fadeRightMini" : "fadeLeftMini"} ${reverse ? styles.textRight : styles.textLeft}`}>
                ...
                {showBtn && (
                    <Button
                      ...
                      Link="https://expo.dev/@mmarayef/react_native_app?serviceType=classic&distribution=expo-go"
                    />
                )}
            </div>

            <div className={`flex-1 ${styles.flexCenter} p-8 sm:px-9`}>
              <img src={mockupImg} className={`${styles.sectionImg} ${reverse ? "fadeLeftMini" : "fadeRightMini"}`} />
            </div>
        </div>
    </div>
  )
